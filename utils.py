...
def load_data(name="full"):
    ...
    if name == "full":
        df = pd.read_csv(DATA_DIR / "catechol_full_data_yields.csv")
        X = df[INPUT_LABELS_FULL_SOLVENT]

    else:
        df = pd.read_csv(DATA_DIR / "catechol_single_solvent_yields.csv")
...
def load_features(name="spange_descriptors"):
    ...
    features = pd.read_csv(DATA_DIR / f"{name}_lookup.csv", index_col=0)
    return features


def generate_leave_one_out_splits(X: pd.DataFrame, Y: pd.DataFrame) -> Generator[
...
def generate_leave_one_ramp_out_splits(X: pd.DataFrame, Y: pd.DataFrame) -> Generator[
    tuple[tuple[pd.DataFrame, pd.DataFrame], tuple[pd.DataFrame, pd.DataFrame]],
    Any,
    None,
]:
